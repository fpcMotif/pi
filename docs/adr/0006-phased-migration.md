...
2. **`0.76.0`** — narrow supported providers in `pi-ai` from 28 to OpenAI / OpenRouter / OpenAI Codex (ADR-0003). Refresh `models.generated.ts` for the narrowed provider set. Update `defaultModelPerProvider`, provider display names, docs, CHANGELOGs. Ships from main as a major-y minor.
