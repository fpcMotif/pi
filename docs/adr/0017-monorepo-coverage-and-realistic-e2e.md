...
CI enforces per-package thresholds via `npm run test:coverage:100 --workspaces` plus a root `coverage:all` aggregator that prints a unified report and exits non-zero if any package falls below 100%. No pre-commit hook (coverage is too slow for pre-commit; ~5-30s per package × six packages).
