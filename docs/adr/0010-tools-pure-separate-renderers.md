...
Each tool handler Effect returns a typed result validated against the tool's `success` schema. The agent event stream's `ToolCompleted` event carries this typed value. The LLM-facing serialization is produced from the same Schema-encoded value — the LLM sees JSON, the UI sees the typed value, both come from one Schema.
