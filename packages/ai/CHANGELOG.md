...
- **Provider narrowing — supported provider set drops from 28 to 3** (ADR-0003, ADR-0006 phase 2). The retained providers are **OpenAI**, **OpenAI Codex Responses**, and **OpenRouter**. Removed: native Anthropic, Amazon Bedrock, Google direct, Google Vertex, Azure OpenAI Responses, DeepSeek, GitHub Copilot, xAI, Groq, Cerebras, Vercel AI Gateway, Z.AI, Mistral, MiniMax (incl. CN), Moonshot AI (incl. CN), Hugging Face, Fireworks, Together AI, OpenCode (Zen/Go), Kimi-for-Coding, Cloudflare Workers AI, Cloudflare AI Gateway, Xiaomi MiMo and its token-plan regional variants. Provider impl files, env-key auto-detection for removed providers, OAuth flows for Anthropic and GitHub Copilot, and the model registry entries for all removed providers are gone. Claude is still reachable via OpenRouter but native prompt-cache control, thinking-block fidelity, and beta features are not preserved.
- Public type surface narrowed: `KnownApi` collapses to `"openai-completions" | "openai-responses" | "openai-codex-responses"`; `KnownProvider` collapses to `"openai" | "openai-codex" | "openrouter"`. The `AnthropicMessagesCompat` interface and `VercelGatewayRouting` interface are removed (with the matching `vercelGatewayRouting` field on `OpenAICompletionsCompat`). The `Model<TApi>["compat"]` conditional drops the `anthropic-messages` branch.
- Provider subpath exports trimmed: `./anthropic`, `./azure-openai-responses`, `./google`, `./google-vertex`, `./mistral`, `./bedrock-provider` are removed from `package.json` exports. Kept: `./openai-codex-responses`, `./openai-completions`, `./openai-responses`, `./oauth`.
- Provider SDK dependencies removed: `@anthropic-ai/sdk`, `@aws-sdk/client-bedrock-runtime`, `@google/genai`, `@mistralai/mistralai`.
- Generator script `scripts/generate-models.ts` is removed (it referenced deleted modules and the dropped provider sources). The narrowed `packages/models/src/models.generated.ts` is hand-maintained until a slim Effect-shaped generator is rebuilt post-rewrite. `scripts/generate-image-models.ts` survives (only fetches from OpenRouter, which is still supported).
- Provider-matrix tests (`abort`, `cache-retention`, `context-overflow`, `empty`, `stream`, `tokens`, `total-tokens`, `tool-call-without-result`, `image-tool-result`, `interleaved-thinking`, `unicode-surrogate`, `xhigh`, `supports-xhigh`, `overflow`, `responseid`, `cross-provider-handoff`) and all provider-specific tests for removed providers (Anthropic, Azure, Bedrock, Google, Mistral, Fireworks, Together, GitHub Copilot, etc.) are deleted. Tests will be rebuilt against `pi-agent-core/test-support` in phase 4 per ADR-0015.
