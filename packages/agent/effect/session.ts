...
/**
 * `Session.send` is overloaded on the toolkit value so callers passing a
 * concrete `Toolkit.make(GetWeather, ...)` get the precise inferred services.
 * Effect's own `LanguageModel.streamText` has the same split: a no-toolkit
 * overload and a `ToolkitOption<any>` overload. Using
 * `ToolkitInput<Record<string, Tool.Any>>` here would be too wide
...
export interface Session {
	...
	readonly send: {
		...
		<ToolkitValue extends LanguageModel.ToolkitOption<any>>(
			input: string | Input,
...
export const makeSession = (options: MakeOptions & SessionConfig): Effect.Effect<Session> =>
	Effect.gen(function* () {
		...
		function send<ToolkitValue extends LanguageModel.ToolkitOption<any>>(
			input: string | Input,
			toolkit: ToolkitValue,
			concurrency?: Types.Concurrency,
		): Stream.Stream<AgentEvent, AgentError, LanguageModel.LanguageModel | ToolkitServices<ToolkitValue>>;
		function send(
			input: string | Input,
			toolkit?: LanguageModel.ToolkitOption<any>,
			concurrency?: Types.Concurrency,
		) {
			...
			return Stream.unwrap(
				Effect.gen(function* () {
					...
					const applyInputHistory = yield* (() => {
						switch (normalized._tag) {
							case "NewPrompt":
								return Effect.succeed((history: Prompt.Prompt) => Prompt.concat(history, normalized.prompt));
							case "AcceptedPromptEnvelope":
								return promptFromAcceptedEnvelope(normalized).pipe(
									Effect.map((acceptedEnvelopePrompt) => (history: Prompt.Prompt) => {
										const withEnvelope = Prompt.concat(history, acceptedEnvelopePrompt);
							...
							case "Retry":
								return Effect.succeed(rollbackToLastUserMessage);
							default:
								return Effect.succeed((history: Prompt.Prompt) => history);
						}
					})();

					yield* SubscriptionRef.update(state, (s) => SessionState.advance(s, applyInputHistory(s.history)));
