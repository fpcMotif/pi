...
Thirty-nine tracer bullets, all GREEN, all without an API key:
...
33.   **`SessionStore` + `Session.durable(id)` for durable state side effects** (ADR-0012 store boundary inside `pi-agent-core` — landed on `main` in parallel with this branch's slices 17–32; merged in here). New `effect/stores/session-store.ts` defines:
    ...
    `effect/session.ts` now exposes `Session.durable(id)`, which loads the previous `SessionState` from `SessionStore`, persists the post-bump snapshot on every `send` (the once-per-send pre-stream side effect, alongside the in-memory `state` ref update), then continues through the existing `LanguageModel.streamText` Effect AI path. 24 tests cover the store boundary, durable session reload, and KV/schema error mapping:
...
38.   **Structured-checkpoint summary prompt** (slice 38 — closes out a compaction follow-on). `SUMMARIZATION_INSTRUCTION` in `effect/session.ts` is upgraded from the one-line "preserve goals, decisions, paths, next steps" prose to an explicit markdown-section template:
    ...
    The instruction is prefixed with a preservation preamble: "Preserve exact file paths, function names, error messages, and any constraints or blockers verbatim — the conversation prefix being summarised will be DISCARDED, so anything not captured here is lost." Empty sections are omitted by the model rather than padded. The summarisation flow is unchanged — the new instruction is appended as the trailing user message of the summary call's prompt, exactly as before — only the *content* of that instruction changes. See ADR-0019 for the design evolution (initial four-section simplification was corrected to five sections + preservation preamble after codex adversarial review surfaced a regression where the compacted-away history's load-bearing facts were permanently lost).
...
39.   **Lifecycle-hooks composition adapter `composeHooks`** (slice 39 — closes the ADR-0014 host-lifecycle composition follow-on from slice 37). New free function on `effect/hooks.ts`:


    ```ts
    export const composeHooks = (...hooks: ReadonlyArray<Hooks>): Hooks => ({
      onAgentEvent: (event) => Effect.forEach(hooks, (h) => h.onAgentEvent(event), { discard: true }),
      onStart:      (input) => Effect.forEach(hooks, (h) => h.onStart(input),      { discard: true }),
      onShutdown:   (exit)  => Effect.forEach(hooks, (h) => h.onShutdown(exit),    { discard: true }),
    })
    ```

    Sequential fan-out with defect-isolation: each hook method on the composed result invokes every underlying `Hooks` in pass-order. `Effect.forEach` defaults to sequential and the `discard: true` option drops the per-call result, so the composed hook's signature stays identical to `Hooks`. Hooks are observer-only on the typed path (`Effect<void, never, never>` — typed failures impossible). **Defects** (`Effect.die`, unchecked throws — including **synchronous throws** thrown while constructing the hook's Effect, thanks to an `Effect.suspend` thunk around each invocation) inside an underlying hook are absorbed via `Effect.catchDefect` and logged at warning level; subsequent hooks for that event STILL run, and the loop completes unaffected. This isolates observer bugs from the loop so a buggy logging / telemetry / UI hook cannot leave a durable session in a partial-turn state. See ADR-0018 for the design evolution (initial fail-fast was reversed after codex adversarial review surfaced the durable-partial-turn blast radius; a follow-up review caught and closed the synchronous-throw bypass). `composeHooks()` (no args) yields a no-op `Hooks` equivalent to the `Context.Reference` default.

    5 tests in `test/effect/hooks-compose.test.ts`:

    - Composing two `recordingHooks()` sinks via `composeHooks(first.hooks, second.hooks)` makes both record every event in stream order, both see exactly one `onStart({_tag: "NewPrompt"})`, and both see exactly one `Success`-shaped `onShutdown`.
    - `composeHooks()` (zero args) leaves the loop behaving exactly like the no-op default; the stream completes normally and `R` does not gain a `Hooks` requirement.
    - Two labelled tag-hooks composed `a → b` show start fanning `a:start, b:start` and shutdown fanning `a:shutdown, b:shutdown`, and every event fans `a:event, b:event` in that order — locking the sequential pass-order contract.
    - **Defect-isolation contract** (ADR-0018): a hook that `Effect.die`s on `onAgentEvent` has its defect absorbed by `composeHooks` and logged at warning level; the second hook STILL sees every `AgentEvent` (subsequent hooks for that event are NOT skipped); the loop completes successfully (`exit._tag === "Success"`); `onStart` still fans `a → b` before the die; `onShutdown` still fans through both via `Stream.onExit`. Locks in the "observer defects can't leave a durable partial turn" semantic.
    - **Synchronous-throw isolation** (ADR-0018, follow-up review): a hook whose `onAgentEvent` does a raw `throw new Error(...)` (instead of returning `Effect.die(...)`) is also isolated, because `composeHooks` invokes each hook inside `Effect.suspend(() => h.onX(arg))` — the synchronous throw lands as a defect inside the Effect runtime and `Effect.catchDefect` absorbs it identically. Same five assertions as the `Effect.die` case. Without the `Effect.suspend` guard, a synchronous throw would bypass `Effect.catchDefect` because the throw happens before the wrapper runs.

    **Still deferred (within hooks)**: mutating hooks for tool-call approval / result patching (the hook can block or replace a tool call before / after it runs). When typed errors become possible (i.e. mutating hooks gain a non-`never` error channel), the composition semantic for *typed* failures will need its own design pass — ADR-0018 explicitly leaves that open.
...
- **Hooks** — `effect/hooks.ts`. `Context.Reference` service with three observer methods (no tool-call interception yet — deferred):
   - `onStart(input: Input)` — fires once before the upstream stream opens.
   - `onShutdown(exit: Exit<unknown, AgentError>)` — fires exactly once on stream completion (success or failure).
   - `onAgentEvent(event: AgentEvent)` — fires per emitted AgentEvent in stream order.
     Default is a no-op `Context.Reference`. Public type `Hooks` is in `effect/hooks.ts`.
- **composeHooks** — `effect/hooks.ts`. Free function `composeHooks(...hooks: ReadonlyArray<Hooks>): Hooks` returning a fan-out hook: each method on the composed result invokes every underlying `Hooks` in pass-order. Sequential (`Effect.forEach` default), `discard: true`. Defects (`Effect.die`, unchecked throws) inside an underlying hook are absorbed via `Effect.catchDefect` and logged at warning level; subsequent hooks for that event STILL run, and the loop completes unaffected (defect-isolation, per ADR-0018). `composeHooks()` (no args) yields a no-op equivalent to the `Context.Reference` default.
...
- **recordingLanguageModelDual** — `test-support/recording-language-model-dual.ts`. Returns `{ layer, calls, summaryCalls }`. The recording sibling of `stubLanguageModelDual` AND of `recordingLanguageModelStream`: it records both `streamText` invocations (in `calls`) AND `generateText` invocations (in `summaryCalls`). Use when a test must assert on the prompt `Session.send` passes to `streamText` in a flow that ALSO triggers compaction (which calls `generateText`) — e.g. the slice-38 structured-checkpoint instruction landing in the summary call's prompt. `generateText` answers with `options.summaryText`; `streamText` yields `options.streamParts`; `generateObject` dies.
...
- **Session** — `effect/session.ts`. The public ADR-0009 entry point for the pi agent loop. `Session.empty` builds a fresh session; `session.send(input, toolkit?, concurrency?): Stream<AgentEvent, AgentError, LanguageModel.LanguageModel>` runs one turn (or a `Retry` / `Continue` follow-on). `session.state: SubscriptionRef<SessionState>` exposes the observable snapshot. The file-header JSDoc on `session.ts` documents the per-slice mechanics (history accumulation, retry, telemetry, compaction, lifecycle hooks, etc.); this entry just names the entry points.
- **SessionState** — `effect/session-state.ts`. `Schema.Class` holding the per-session observable snapshot: `turnCount`, `history: Prompt.Prompt`, `inputTokens`, `outputTokens`, `compactionCount`. Statics: `.empty` (zero-valued), `.with({ partial-patch })` (single update point so callers can't drop a field as new ones are added), `.advance(history)` (`turnCount + 1` + new history; preserves token totals and `compactionCount`).
- **SessionStore** — `effect/stores/session-store.ts`. `Context.Service` for durable session persistence (ADR-0012). Methods: `save(id, state)`, `load(id): Option<SessionState>`, `remove(id)`, `list: ReadonlyArray<string>`. All return Effects with `SchemaError | StoreError` failure channels. The on-disk envelope is `SessionRecordV1` (`{ version: 1, state, updatedAt }`); tests provide an in-memory Layer, host code provides a KeyValueStore-backed Layer.
- **Input / NewPrompt / Continue / Retry / AcceptedPromptEnvelope** — `effect/agent-input.ts`. Tagged union accepted by `Session.send` (ADR-0009). `NewPrompt({ prompt })` — fresh user turn, appends to history. `Continue` — re-open the upstream with the existing history (no new user message). `Retry` — roll back to the last user message, drop the trailing assistant turn, re-run. `AcceptedPromptEnvelope({ content, injectedMessages?, queueMode?, source?, systemPromptOverride? })` — a host-preflighted turn (skills, prompt templates, extension transforms already applied). `Session.send` also accepts a bare `string`, normalised to `NewPrompt({ prompt })` via `normalize`.
...
- ADR-0009 wrapping follow-ons: lifecycle observer hooks `onStart(input)` and `onShutdown(exit: Exit<unknown, AgentError>)` landed in slice 37 alongside the existing `onAgentEvent`, with the stream's typed `Exit` carried through to the host via `Stream.onExit`. `composeHooks(...hooks)` landed in slice 39 as the host-lifecycle composition adapter. Still pending: mutating hooks for tool-call approval / result patching. Skill-block parsing was **removed** from this target — it is a host (`pi-coding-agent`) concern (ADR-0009 amendment, 2026-05-14); the loop consumes already-expanded prompts.
