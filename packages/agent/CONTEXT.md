...
- **CurrentSession** — `effect/current-session.ts`. The ADR-0009 `Session` promoted to a `Context.Service` (ADR-0020 decision 4; the consolidation-backlog P0). `layerEphemeral(config?)` / `layerDurable(sessionId)` build one session per layer scope; hosts resolve it from the ADR-0008 ManagedRuntime, tests swap in fakes via `Layer.effect(CurrentSession, …)`. A multi-session registry (newSession/fork/switch) is deliberately deferred until interactive mode crosses the seam.
- **providers/openai** — `effect/providers/openai.ts`. Production OpenAI provider wiring (ADR-0003/0005): `openAiLanguageModelLayer({ model, apiKey, apiUrl? })` composes `OpenAiLanguageModel.layer ⟵ OpenAiClient.layer` (Redacted key) `⟵ FetchHttpClient.layer`; `openAiLanguageModelLayerHttp` leaves the `HttpClient` requirement open so tests drive the genuine HTTP path with `stubHttpClient`.
- **`/effect` package export** — `effect/index.ts` is the lane's public barrel, built to `dist/effect` by `tsconfig.effect-build.json` and exported as `@earendil-works/pi-agent-core/effect`. First external consumer: pi-coding-agent's `--effect` print adapter (ADR-0020). Internal pipeline modules (attempt-stream, lift-part, history-accumulator, token-capture, retry) are deliberately not re-exported.
