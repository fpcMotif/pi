...
- Set `bunCommand` in `settings.json` to pin registry package lookup and install operations to a specific Bun invocation. The first argv entry must be `bun`.
...
- Runs `bun install --production` after clone or pull if `package.json` exists.
